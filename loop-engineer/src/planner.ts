...
export async function planSpec(
  ...
  // CC-58：默认 planner=workers-ai（含额度），降级链默认到 hilinkup（配额用尽/限流兜底）。
  // 仍不把 claude 放进默认链（本机订阅，云端无 login 一触发即报错）。
  const primary = config.providers.planner;
  const fallbacks = (process.env.LOOP_PLANNER_FALLBACK ?? "hilinkup:kimi-k2.5")
    .split(",")
    .map((s) => s.trim())
    .filter(Boolean);
